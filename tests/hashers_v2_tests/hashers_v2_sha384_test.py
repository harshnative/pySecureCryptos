...
def test_compatible():
    ...
    fileName = "binFiles/hashers_v2_testcases_bin/sha384_test_testCases.bin"
    ...
    for byte , hashed_byte , hashed_byte_yield , hashed_string , hashed_string_yield in pickledList:
        ...
        new_hashed_byte = hashers_v2.SHA384(byte).get_byte()

        assert new_hashed_byte == hashed_byte , getAssetionMessage(locals() , "hashed bytes does not match") 


        genObj = hashers_v2.SHA384(byte).get_byte_yield()
        ...
        new_hashed_string = hashers_v2.SHA384(byte).get_string()

        assert new_hashed_string == hashed_string , getAssetionMessage(locals() , "hashed string does not match") 


        genObj = hashers_v2.SHA384(byte).get_string_yield()
