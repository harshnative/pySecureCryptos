...
# function to test Base64_64 yield encode with Base64_64 decode
@pytest.mark.repeat(repeatTimes.RepeatTime.value)
def test_main():

    byte = getRandomByte()

    genObj = encoderDecoders.Base64_64.encode_yield(byte)
    ...
    byteAgain = encoderDecoders.Base64_64.decode(stringFromByte)
...
# function to test Base64_64 encode with Base64_64 yield decode
@pytest.mark.repeat(repeatTimes.RepeatTime.value)
def test_main_2():

    byte = getRandomByte()

    stringFromByte = encoderDecoders.Base64_64.encode(byte)

    genObj = encoderDecoders.Base64_64.decode_yield(stringFromByte)
...
# function to test Base64_64 yield encode with Base64_64 yield decode
@pytest.mark.repeat(repeatTimes.RepeatTime.value)
def test_main_3():
...
def test_compatible_1():
    ...
    fileName = "binFiles/encoderDecoder_testcases_bin/Base64_64_yield_test_testCases.bin"
    ...
    for byte , encodedByte in pickledList:
        ...
        byteAgain = encoderDecoders.Base64_64.decode(encodedByte)
...
def test_compatible_2():
    ...
    for byte , encodedByte in pickledList:
        ...
        gen = encoderDecoders.Base64_64.encode_yield(byte)
...
def test_compatible_3():
    ...
    for byte , encodedByte in pickledList:
        ...
        gen = encoderDecoders.Base64_64.decode_yield(encodedByte)
