...
from pySecureCryptos.verifier_fernetWrapper import BytesEncryptor
...
def byteEncryptorTest_func(howManyTimes , writeErrors = True):
    ...
    for k in range(howManyTimes):
        ...
        fernetObj = BytesEncryptor(randString1)

        # executing the functions to test and calculating time
        startTime = time.perf_counter()
        encrytedByte = fernetObj.encrypt(randomByte)

        decryptedByte = fernetObj.decrypt(encrytedByte)
