...
from pySecureCryptos.verifier_fernetWrapper import StringEncryptor
...
def StringEncryptorTest_func(howManyTimes , writeErrors = True):
    ...
    for k in range(howManyTimes):
        ...
        genObj_encrypt = StringEncryptorObj.encrypt_yield(randString2 , False)
        ...
        genObj_decrypt = StringEncryptorObj.decrypt_byte_yield(encrytedByte)
        while(True):
            try:
        ...
        if(decryptedString != randString2):
            errorList.append([randString2 , encrytedByte , decryptedString])
