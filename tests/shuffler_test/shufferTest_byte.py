...
def shufflerTester_Byte(howManyTimes , writeErrors = True):
    avgTime = 0
    errorList = []
    totalErrors = 0
    ...
    for k in range(howManyTimes):
        ...
        # byte size
        byteLength = random.randint(1 , 1000)

        # byte for shuffler test
        randomByte = secrets.token_bytes(byteLength)


        # string for shuffle test
        randList = random.choices(mainList , k=byteLength)
        randString2 = "".join(randList)

        # executing the functions to test and calculating time
        startTime = time.perf_counter()
        shuffledByte = Shuffler.shuffle_byte(randomByte , randString2)

        deShuffledByte = Shuffler.unShuffle_byte(shuffledByte , randString2)
        ...
        if(deShuffledByte != randomByte):
            errorList.append([randString2 , shuffledByte , deShuffledByte])
