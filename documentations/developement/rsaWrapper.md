...
```python
encObj = RW.Encryptor(publicKey , privateKey , keySize = 0)
```
...
* keySize -> Default is set to 0, means module will auto try to find the keySize. If the module fails it will raise a value error. then you need to pass the keySize in KeyGenerator(). Wrong key size may lead to errors in encryption saying plain text is too long etc.
